 ...
 int main()
 {
    ...
    while (true)
    {
        for (size_t i=0; i<filenames.size(); i++)
        {
            ...
            //threshold(imgThresholded,imgThresholded, thresholdVal, thresholdMax, thresholdMode);

         //   inRange(imgOriginal, Scalar(iLowH, iLowS, iLowV), Scalar(iHighH, iHighS, iHighV), imgThresholded); //Threshold the image
            imshow("Thresholded", imgThresholded);

            //erode(imgThresholded, imgThresholded, getStructuringElement(MORPH_RECT, Size(10, 10)));
            //imshow("final", imgThresholded);
       ...
       //     inRange(imgProduct, Scalar(80, 0, 0), Scalar(150, 255, 255), imgProduct); //Threshold the image
            ...
            vector<int> labels(lines.size(), 0);
            for( size_t i = 0; i < lines.size(); i++ )
            {
                if (labels[i] != 0)
                    continue;
                ...
                for (size_t j = i+1; j < lines.size(); j++)
                {
                    ...
                    if (abs(lines[i][0] - lines[j][0]) < 20 && abs(lines[i][1] - lines[j][1]) < 0.2)
                    {
                        labels[j] = 1;
                        matches.push_back(lines[j]);
                ...
                if (matches.size() > 1)
                {
                    ...
                    Vec2f trueLine(rho / (float)matches.size(), theta / (float)matches.size());
                    trueLines.push_back(trueLine);
                }
            ...
            {
                line( imgOriginal, p[i][0], p[i][1], Scalar(0,0,255), 1, CV_AA);
            ...
            vector<int> labels2(intersections.size(), 0);
            ...
            for (size_t i = 0; i < intersections.size(); i++)
            {
                ...
                mean_vals.push_back(mean(imgThresholded(ROI))[0]);
            }
            vector<int> mean_vals_sorted;
            cv::sort(mean_vals, mean_vals_sorted, SORT_EVERY_ROW);
            for (size_t i = 0; i < intersections.size(); i++)
            {
                if (mean_vals[i] <= mean_vals_sorted[5])
                    drawMarker(imgOriginal, intersections[i], Scalar(0, 255, 0), MARKER_CROSS);
                else

                    drawMarker(imgOriginal, intersections[i], Scalar(0, 0, 255), MARKER_CROSS);
            }
